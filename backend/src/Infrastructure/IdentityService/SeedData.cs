...
public class SeedData
{
    public static void EnsureSeedData(WebApplication app)
    {
        ...
        if (alice == null)
        {
            ...
            var result = userMgr.CreateAsync(alice, "Admin_1234").Result;
            if (!result.Succeeded)
            {
        ...
        if (bob == null)
        {
            ...
            var result = userMgr.CreateAsync(bob, "Admin_1234").Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            }
        ...
        if (tom == null)
        {
            ...
            var result = userMgr.CreateAsync(tom, "Admin_1234").Result;
            if (!result.Succeeded)
            {
                throw new Exception(result.Errors.First().Description);
            }
